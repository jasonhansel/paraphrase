...
fn change_char<'s>(args: Vec<Value<'s>>) -> NativeResult<'s> {
    match get_args(args) {
...
fn if_eq<'s>(args: Vec<Value<'s>>) -> NativeResult<'s> {
    match get_args(args) {
...
fn if_eq_then<'s>(args: Vec<Value<'s>>) -> NativeResult<'s> { 
    match get_args(args) {
        (Some(value_a), Some(value_b), Some(Closure(if_true)), Some(Closure(if_false)), Some(Closure(mut finally)), None,..) => {
            let todo = (if value_a == value_b { if_true } else { if_false }).force_clone().1;
            Continue(Rope::new(), finally.0.clone(), todo.concat(* finally.force_clone().1))
        },
        _ => {panic!()}
    }
}

fn bubble<'s>(args: Vec<Value<'s>>) -> NativeResult<'s> {
    match get_args(args) {
...
fn end_paren<'s>(args: Vec<Value<'s>>) -> NativeResult<'s> {
    match get_args(args) {
        (None, ..) => {
...
fn literal<'s>(args: Vec<Value<'s>>) -> NativeResult<'s> {
    match get_args(args) {
        (Some(Closure(ValueClosure(_, closure))), None, ..) => {
            Done(Value::Str(closure.to_str().unwrap()))
        },
        _ => { panic!() }
    }
}
...
fn define<'s>(args: Vec<Value<'s>>) -> NativeResult<'s> {
    match get_args(args) {
        (Some(Str(name_args)),
       ...
       Some(Closure(ValueClosure(_, to_expand))), None, ..) => {
            ...
            Continue(Rope::new(), new_scope, *to_expand)
        },
        args => {
...
fn expand<'s>(args: Vec<Value<'s>>) -> NativeResult<'s> {
    match get_args(args) {
        (Some(Closure(ValueClosure(scope, contents))), None, ..) => {
            Continue(Rope::new(), scope.clone(), *contents )
        },
        _ => {panic!("ARG"); }
    }
}

fn rescope<'s>(args: Vec<Value<'s>>) -> NativeResult<'s> {
    match get_args(args) {
